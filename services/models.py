...
class ServiceMechanic(models.Model):
    ...
    def active_bookings_count(self):
        return self.booking_assignments.filter(
            booking__status__in=['confirmed', 'in_progress']
        ).count()

    def completed_bookings_count(self):
        return self.booking_assignments.filter(
            booking__status='done'
        ).count()
...
class ServicePart(models.Model):
    ...
    cost_price = models.DecimalField(
        max_digits=8, decimal_places=2, null=True, blank=True,
        verbose_name='Preț achiziție (RON)'
    )
    sale_price = models.DecimalField(
        max_digits=8, decimal_places=2, null=True, blank=True,
        verbose_name='Preț vânzare (RON)'
    )
