...
def service_dashboard(request):
    ...
    bookings_qs = Booking.objects.filter(center__in=centers).select_related('center', 'service_item', 'user', 'garage').prefetch_related('attachments').order_by('-created_at')
...
def bookings_list(request):
    ...
    bookings = Booking.objects.filter(
        ...
        'center', 'garage'
    ).prefetch_related('mechanics__service_mechanic').order_by('-booking_date', '-booking_time')
...
def booking_detail(request, pk):
    booking = get_object_or_404(
        Booking.objects.select_related('center', 'service_item', 'user', 'garage').prefetch_related('attachments'),
        pk=pk,
    )
    ...
    if request.method == 'POST':
        ...
        if action == 'save_work_order_info':
            ...
            booking.save(update_fields=['additional_description', 'updated_at'])
        ...
        if action == 'assign_mechanic':
            from bookings.models import BookingMechanic
            ...
            if mechanic_id:
                ...
                bm, created = BookingMechanic.objects.get_or_create(
                    booking=booking,
                    service_mechanic=mechanic,
                    defaults={'is_primary': is_primary},
                )
                if not created and bm.is_primary != is_primary:
                    bm.is_primary = is_primary
                    bm.save(update_fields=['is_primary'])
                if booking.user:
                ...
                messages.success(request, f'Mecanicul {mechanic.name} a fost alocat programării.')
            else:
                BookingMechanic.objects.filter(booking=booking).delete()
                messages.success(request, 'Mecanicii au fost eliminați de la această programare.')
            return redirect('services:booking_detail', pk=booking.pk)
...
def booking_print(request, pk):
    booking = get_object_or_404(
        Booking.objects.select_related('center', 'service_item', 'user', 'garage'),
        pk=pk,
    )
    if not (request.user.is_staff or booking.center.owner_id == request.user.id):
        return redirect('core:home')
...
def service_car_history(request):
    ...
    bookings = Booking.objects.filter(
        center__in=centers,
        status=Booking.STATUS_DONE,
    ).select_related('center').order_by('-booking_date', '-booking_time', '-created_at')
...
def mechanic_profile(request, pk):
    ...
    active_bookings = Booking.objects.filter(
        mechanics__service_mechanic=mechanic, status__in=['confirmed', 'in_progress']
    ).select_related('center', 'service_item', 'garage').distinct().order_by('booking_date', 'booking_time')

    done_bookings = Booking.objects.filter(
        mechanics__service_mechanic=mechanic, status='done'
    ).select_related('center', 'service_item').distinct().order_by('-booking_date')[:20]
    ...
    if request.method == 'POST':
        ...
        if action == 'save_work_log':
            ...
            booking = get_object_or_404(Booking, pk=booking_id, mechanics__service_mechanic=mechanic)
        ...
        if action == 'update_status':
            ...
            if new_status in allowed:
                ...
                if new_status == 'in_progress':
                    wl, _ = MechanicWorkLog.objects.get_or_create(booking=booking, defaults={'mechanic': mechanic})
