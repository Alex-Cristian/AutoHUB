...
def create_notification_on_new_booking(sender, instance: Booking, created: bool, **kwargs):
    """When a booking is created, notify the service owner after commit."""
    if not created:
        return
    ...
    def _notify_owner():
        BookingNotification.objects.create(
            ...
            title=f"Programare noua #{instance.pk} - {instance.client_name}",
            message=(
                f"Service: {instance.center.name}\n"
                f"Data/Ora: {instance.booking_date} {instance.booking_time}\n"
                f"Masina: {instance.car_brand} {instance.car_model} ({instance.car_plate})\n"
