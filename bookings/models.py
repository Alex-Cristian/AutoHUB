...
class Booking(models.Model):
    ...
    cancellation_reason = models.TextField(blank=True, verbose_name='Motiv anulare')
    cancelled_at = models.DateTimeField(null=True, blank=True, verbose_name='Anulat la')
...
class BookingMechanic(models.Model):
    booking = models.ForeignKey(
        Booking, on_delete=models.CASCADE,
        related_name='mechanics', verbose_name='Programare'
    )
    service_mechanic = models.ForeignKey(
        'services.ServiceMechanic', on_delete=models.CASCADE,
        related_name='booking_assignments', verbose_name='Mecanic'
    )
    ...
    def __str__(self):
        ...
        return f"{self.service_mechanic.name}{primary} → #{self.booking_id}"


class BookingAttachment(models.Model):
    STAGE_CHOICES = [
        ('before', 'Înainte de reparație'),
        ('during', 'În timpul reparației'),
        ('after', 'După reparație'),
    ]
    ROLE_CHOICES = [
        ('client', 'Client'),
        ('mechanic', 'Mecanic'),
    ...
    attachment_stage = models.CharField(
        max_length=10, choices=STAGE_CHOICES, default='before',
        verbose_name='Etapă'
    )
    ...
    is_visible_to_client = models.BooleanField(default=True, verbose_name='Vizibil pentru client')
