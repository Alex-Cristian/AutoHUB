...
from .forms import RegisterForm, LoginForm, CarForm, CarExpiryCalendarForm
...
@login_required
def car_calendar(request, pk):
    car = get_object_or_404(Car, pk=pk, owner=request.user)
    calendar, _ = CarExpiryCalendar.objects.get_or_create(car=car)

    if request.method == 'POST':
        form = CarExpiryCalendarForm(request.POST, instance=calendar)
        if form.is_valid():
            form.save()
        ...
        form = CarExpiryCalendarForm(instance=calendar)
    ...
    return render(request, 'accounts/car_calendar.html', {
        ...
        'form': form,
        'items': items,
        'completion_count': calendar.completion_count(),
        'nearest_expiry': calendar.nearest_expiry(),
        'overall_status': calendar.overall_status(),
    })
